...
void tg_new_msg(struct tgl_state *TLS, struct tgl_message *M)
{
	...
	if (!temp_msg)
		LOGE("memory allocation failed!!! for tgl_message_get");
}
	...
	if (M && TLS->started) {
		...
		if (tgl_get_peer_type(M->to_id) == TGL_PEER_USER) {
			if (M->out) {
				...
				if (M->from_id.id == 333000 || M->from_id.id == 777000)
					M->service = 0;

				char* tb_name = get_table_name_from_number(M->from_id.id);
				Eina_Bool is_present_in_db = is_user_present_buddy_table(M->from_id.id);
				create_buddy_msg_table(tb_name);
				if (!is_present_in_db) {
...
void on_peer_chat_info_received(struct tgl_state *TLS, void *callback_extra, int success, struct tgl_chat *chat_info)
{
	char *msg_table;
...
void on_contacts_and_chats_loaded(struct tgl_state *TLS, void *callback_extra, int success, int size, tgl_peer_id_t peers[], int last_msg_id[], int unread_count[])
{
	...
	for (int i = size - 1; i >= 0; i--) {
		...
		tg_data->peer_list = eina_list_append(tg_data->peer_list, UC);
		// insert into peer table
		switch (tgl_get_peer_type(peers[i])) {
			case TGL_PEER_USER:
				tg_data->buddy_list = eina_list_append(tg_data->buddy_list, UC);

				// check peer exists in peer table / buddy table
				Eina_Bool is_buddy_present = is_user_present_buddy_table(UC->id.id);
				if (!is_buddy_present) {
...
void update_user_display_name(tg_engine_data_s *tg_data, int buddy_id, const char *first_name, const char *last_name)
{
	if (first_name && last_name) {
		tgl_do_set_profile_name(tgl_engine_get_TLS(),
				first_name, last_name, on_profile_name_changed, tg_data);
	}
}
...
extern void on_selected_group_chats_delete_reponse(struct tgl_state *TLS, void *callback_extra, int success, struct tgl_message *M);
static Eina_Bool on_async_chat_deletion_requested(void *data)
{
	Eina_List *sel_grp_chats = data;
	if (sel_grp_chats) {
		tg_engine_data_s *tg_data = tgl_engine_get_TLS()->callback_data;
...
void forward_message_to_buddy(int to_id, int type_of_chat, int from_id, int message_id, int temp_message_id)
{
	char *msg_table = get_table_name_from_number(from_id);
	struct tgl_message* msg = get_message_from_message_table(temp_message_id, msg_table);

	if (msg) {
		tgl_peer_id_t id_to_send;
		id_to_send.id = type_of_chat;
		tgl_do_forward_message(s_info.TLS, id_to_send, message_id, &on_message_sent_to_buddy, (void*)(msg));
	}
	free(msg_table);
}

void send_typing_status_to_buddy(int buddy_id, int type_of_chat, int typing_status)
{
	tgl_peer_id_t id_to_send;
	id_to_send.id = type_of_chat;
	tgl_do_send_typing(s_info.TLS, id_to_send, typing_status, NULL, NULL);
}
