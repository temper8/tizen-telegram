...
{
	if (!ad || (ad && ad->search_peer_list == NULL))
		return;

	user_data_with_pic_s *item = NULL;
	user_data_s* user_data = NULL;
	EINA_LIST_FREE(ad->search_peer_list, item) {
		if (!item)
			continue;
		user_data = item->use_data;
		if (!user_data)
			continue;
...
{
	if (result != APP_CONTROL_RESULT_SUCCEEDED || !user_data) {
		return;
	}
...
static Evas_Object *_get_first_name_content_cb(void *data, Evas_Object *obj, const char *part)
{
	if (0 == strcmp(part, "elm.icon.entry")) {
		...
		appdata_s *ad = evas_object_data_get(obj, "app_data");
		if (ad) {
			if ((ad->is_loading_from_msg_view || ad->is_loading_from_profile_view) && ad->buddy_in_cahtting_data && ad->buddy_in_cahtting_data->use_data && ad->buddy_in_cahtting_data->use_data->first_name) {
				char temp_name[512] = {'\0'};
				snprintf(temp_name, 512, "%s", ad->buddy_in_cahtting_data->use_data->first_name);
				elm_object_text_set(phone_entry, temp_name);
			}
...
static Evas_Object *_get_second_name_content_cb(void *data, Evas_Object *obj, const char *part)
{
	if (0 == strcmp(part, "elm.icon.entry")) {
		...
		appdata_s *ad = evas_object_data_get(obj, "app_data");
		if (ad) {
			if ((ad->is_loading_from_msg_view || ad->is_loading_from_profile_view) && ad->buddy_in_cahtting_data && ad->buddy_in_cahtting_data->use_data && ad->buddy_in_cahtting_data->use_data->last_name) {
				char temp_name[512] = {'\0'};
				snprintf(temp_name, 512, "%s", ad->buddy_in_cahtting_data->use_data->last_name);
				elm_object_text_set(phone_entry, temp_name);
			}
...
Eina_Bool add_contact_to_phone_book(appdata_s *ad)
{
	...
	if (!phone_num || !first_name || !last_name) {
		return EINA_FALSE;
	}
...
void launch_previous_view(appdata_s *ad, int buddy_id)
{
	...
	if (buddy_id > 0) {
		...
		EINA_LIST_FOREACH(ad->main_list, l, main_list_item) {
			if (main_list_item && main_list_item->peer_id == buddy_id) {
				LOGE("on_new_contact_added_response_received3: buddy id: %d", buddy_id);
...
void on_new_contact_added_response_received(appdata_s *ad, int buddy_id, Eina_Bool is_success)
{
	if (is_success) {
		...
		// show failed message
		if (ad->is_loading_from_msg_view || ad->is_loading_from_profile_view) {
			show_toast(ad, "Failed to add contact");
			return;
		}
	}

	Evas_Object *peer_list = evas_object_data_get(ad->nf, "contact_peer_search_list");
	if (!peer_list) {
		elm_naviframe_item_pop(ad->nf);
...
static void on_new_contact_done_clicked(void *data, Evas_Object *obj, void *event_info)
{
	...
	if ((phone_num && strlen(phone_num) > 0) && (first_name && strlen(first_name) > 0) && (last_name && strlen(last_name) > 0)) {
		if (ad->is_loading_from_profile_view || ad->is_loading_from_msg_view || add_contact_to_phone_book(ad)) {
			show_loading_popup(ad);
...
static void on_group_chat_clicked(void *data, Evas_Object *obj, void *event_info)
{
	...
	if (ad && ad->buddy_list && eina_list_count(ad->buddy_list) > 0) {
		...
		char org_msg[1024] = {0,};
		snprintf(org_msg, sizeof(org_msg) - 1, "%s%s%s", msg_str, ". ", i18n_get_text("IDS_TGRAM_MBODY_INVITE_FRIENDS"));
...
static void on_peer_item_clicked(void *data, Evas_Object *obj, void *event_info)
{
	...
	ad->buddy_in_cahtting_data = NULL;
	user_data_with_pic_s *bitem = NULL;
	l = NULL;
	EINA_LIST_FOREACH(ad->buddy_list, l, bitem) {
		if (bitem->use_data->user_id.id == sel_item->use_data->user_id.id) {
			ad->buddy_in_cahtting_data = bitem;
			break;
		}
	}
...
static void _on_search_entry_changed(void *data, Evas_Object *obj, void *event_info)
{
	...
	Evas_Object *search_list = evas_object_data_get(ad->nf, "contact_peer_search_list");
...
void launch_start_peer_search_view(appdata_s* ad)
{
	...
	evas_object_data_set(ad->nf, "contact_peer_search_list", peer_list);
