...
void on_image_select_result_cb(app_control_h request, app_control_h reply, app_control_result_e result, void *user_data)
{
	...
	for (int i = 0 ; i < array_length ; i++) {
		if (path_arryay[i]) {
			...
			send_set_profile_pic_request(ad, ad->service_client, ad->current_user_data->user_id.id, path_arryay[i]);
			break;
		}
	}
}
	...
	if (!strcmp(part, "elm.text.main.left.top") || !strcmp(part, "elm.text")) {
		switch (id) {
			case 0:
				...
				int str_len = strlen(first_name) + strlen(" ") + strlen(last_name) + 1;
				user_name = (char *)malloc(str_len);
				snprintf(user_name, sizeof(user_name), "%s %s", first_name, last_name);
				return user_name;
			case 1:
...
static void _create_more_popup(void *data, Evas_Object *obj, void *event_info)
{
	...
	if (!ad || ad->loading_popup || ad->current_app_state != TG_SETTINGS_SCREEN_STATE)
		return;
