...
{
	if (!is_success || !ad->buddy_in_cahtting_data || ad->buddy_in_cahtting_data->use_data->user_id.id != buddy_id) {
...
static void on_user_block_ok_selected(void *data, Evas_Object *obj, void *event_info)
{
	...
	if (ad->buddy_in_cahtting_data && ad->buddy_in_cahtting_data->use_data) {
		...
		if (get_buddy_block_status(ad->buddy_in_cahtting_data->use_data->user_id.id) == 1) {
			send_unblock_buddy_request(ad, ad->service_client, ad->buddy_in_cahtting_data->use_data->user_id.id);
		} else {
			send_block_buddy_request(ad, ad->service_client, ad->buddy_in_cahtting_data->use_data->user_id.id);
		}
	}
	evas_object_del(popup);
}
...
static void on_user_delete_ok_selected(void *data, Evas_Object *obj, void *event_info)
{
	...
	if (ad->buddy_in_cahtting_data && ad->buddy_in_cahtting_data->use_data) {
		if (get_buddy_delete_status(ad->buddy_in_cahtting_data->use_data->user_id.id) == 1 || get_buddy_unknown_status(ad->buddy_in_cahtting_data->use_data->user_id.id) == 1) {
			launch_add_contact_screen(ad);
		} else {
			show_loading_popup(ad);
			send_delete_buddy_request(ad, ad->service_client, ad->buddy_in_cahtting_data->use_data->user_id.id);
		}
...
void on_user_info_menu_option_selected_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	switch(id) {
	case 0:
		if (ad->buddy_in_cahtting_data->use_data->user_id.type == TGL_PEER_USER) {
			if (get_buddy_delete_status(ad->buddy_in_cahtting_data->use_data->user_id.id) == 1 || get_buddy_unknown_status(ad->buddy_in_cahtting_data->use_data->user_id.id) == 1) {
...
char* on_user_info_menu_text_get_cb(void *data, Evas_Object *obj, const char *part)
{
	...
	switch(id) {
	case 0:
		if (get_buddy_delete_status(ad->buddy_in_cahtting_data->use_data->user_id.id) == 1 ||
				get_buddy_unknown_status(ad->buddy_in_cahtting_data->use_data->user_id.id) == 1)
			return strdup(i18n_get_text("IDS_TGRAM_OPT_ADD_TO_TELEGRAM"));
		else
			return strdup(i18n_get_text("IDS_TGRAM_OPT_DELETE"));

	case 1:
		if(!is_telegram_account(ad->buddy_in_cahtting_data->use_data->user_id.id))
...
Evas_Object* on_buddy_info_requested(void *data, Evas_Object *obj, const char *part)
{
	...
	if (!strcmp(part, "elm.swallow.icon")) {
		...
		if (ad->buddy_in_cahtting_data->use_data->photo_path && strlen(ad->buddy_in_cahtting_data->use_data->photo_path) > 0 && strstr(ad->buddy_in_cahtting_data->use_data->photo_path, "_null_") == NULL) {
			profile_pic = get_image_from_path(ad->buddy_in_cahtting_data->use_data->photo_path, ad->nf);
		} else  {
		...
		char* user_name = replace(ad->buddy_in_cahtting_data->use_data->print_name, '_', " ");
		char buf[512] = {'\0'};
...
Evas_Object* on_buddy_phone_info_requested(void *data, Evas_Object *obj, const char *part)
{
	...
	if (id == 0) {
		if (ad->buddy_in_cahtting_data && ad->buddy_in_cahtting_data->use_data->phone) {
			...
			char *phone_no = get_buddy_phone_num_from_id(ad->buddy_in_cahtting_data->use_data->user_id.id);
			if (phone_no) {
				first_text = phone_no;
			} else {
				first_text = "";
...
{
	if (!ad || !ad->buddy_in_cahtting_data)
		return;
	...
	eext_object_event_callback_del(ad->nf, EEXT_CALLBACK_MORE, on_user_info_menu_button_clicked);
	char edj_path[PATH_MAX] = {0, };
	app_get_resource(TELEGRAM_INIT_VIEW_EDJ, edj_path, (int)PATH_MAX);
	...
	Eina_List *buddy_details_array = get_buddy_info(ad->buddy_in_cahtting_data->use_data->user_id.id);
	...
	char *temp_phone_num = ad->buddy_in_cahtting_data->use_data->phone;
	if (!temp_phone_num || strlen(temp_phone_num) < 2) {
		temp_phone_num = i18n_get_text("IDS_TGRAM_SBODY_UNKNOWN");
	} else {
	...
	sprintf(phone_num_str, "<font=Tizen:style=Bold color=#000000 align=left><font_size=40>&nbsp;&nbsp;%s</font_size></font>", temp_phone_num);
