...
Eina_Bool insert_unsent_message_to_db(const char *app_name, const char *command, const char *buddy_id, const char *msg_id, const char *msg_type, const char *msg_data, const char *type_of_chat)
{
	...
	if (!ret)
		LOGE("db table creation failed for %s", table_name);
...
Eina_List *get_all_peer_details(char *start_name)
{
	...
	char *wh_cs = NULL;
	if (start_name) {
		snprintf(where_clause, sizeof(where_clause), PEER_INFO_TABLE_PRINT_NAME" LIKE '%s%s'", start_name, "%");
		wh_cs = where_clause;
	}
	peer_details = get_values_from_table_sync_order_by(table_name, col_names, col_types, PEER_INFO_TABLE_LAST_MESSAGE_DATE, EINA_FALSE, wh_cs, TG_DBMGR_NOLIMITED, TG_DBMGR_NOLIMITED);
...
peer_with_pic_s* get_peer_info(int peer_id)
{
	...
	sprintf(where_clause, PEER_INFO_TABLE_CHAT_ID" = %d", peer_id);
...
tg_chat_info_s* get_chat_info(int chat_id)
{
	...
	char where_clause[1024];
	sprintf(where_clause, CHAT_INFO_TABLE_CHAT_ID" = %d", chat_id);
...
Eina_List *get_buddy_list_info()
{
	...
	char where_clause[1024];
	sprintf(where_clause, USER_INFO_TABLE_IS_UNKNOWN_PEER" = 0 OR "USER_INFO_TABLE_IS_DELETED" = 0");
...
Eina_List *get_unknown_buddy_list_info()
{
	...
	char where_clause[1024];
	sprintf(where_clause, USER_INFO_TABLE_IS_UNKNOWN_PEER" = 1 OR "USER_INFO_TABLE_IS_DELETED" = 1");
...
char *get_profile_pic_path(int buddy_id)
{
	...
	char where_clause[1024];
	sprintf(where_clause, USER_INFO_TABLE_USER_ID" = %d", buddy_id);
	...
	// buddy id is uniqe, there will be only one value.
...
Eina_Bool is_phone_number_exists_in_buddy_list(const char *phone_num)
{
	...
	char where_clause[1024];
	sprintf(where_clause, USER_INFO_TABLE_PHONE_NO" LIKE '%s%s%s'", "%", phone_num, "%");
...
char *get_buddy_phone_num_from_id(int buddy_id)
{
	...
	if (buddy_details_array && eina_list_count(buddy_details_array) > 0) {
		Eina_List *buddy_details = eina_list_nth(buddy_details_array, 0);
		if (buddy_details && eina_list_count(buddy_details) > 0) {
			char *pn = (char *)eina_list_nth(buddy_details, 0);
			if (pn && strlen(pn) > 0)
				phone_num = strdup(pn);
			free(pn);
			eina_list_free(buddy_details);
...
tg_message_s *get_latest_message_from_message_table(char *table_name, Eina_Bool is_for_date)
{
	...
	message_details = get_values_from_table_sync_order_by(table_name, col_names, col_types, MESSAGE_INFO_TABLE_DATE, EINA_FALSE, wh_c, TG_DBMGR_NOLIMITED, TG_DBMGR_NOLIMITED);
...
Eina_Bool set_all_rows_read(const char *table_name)
{
	...
	char where_clause[1024];
	sprintf(where_clause, MESSAGE_INFO_TABLE_UNREAD" = 1");
...
void update_msg_into_db(tg_message_s *M, char *table_name)
{
	...
	char where_clause[1024];
	sprintf(where_clause, MESSAGE_INFO_TABLE_MESSAGE_ID" = %d", M->msg_id);
...
int insert_current_date_to_table(char *tb_name)
{
	...
	if (last_msg) {
		...
		if (old_lt.tm_mday == new_lt.tm_mday && old_lt.tm_mon == new_lt.tm_mon && old_lt.tm_year == new_lt.tm_year) {
			...
			date_msg.unread = 0;
			date_msg.out = 0;
			date_msg.media_id = g_empty_char;
...
tg_message_s *get_message_from_message_table(long long msg_id, char *table_name)
{
	...
	char where_clause[1024];
	sprintf(where_clause, MESSAGE_INFO_TABLE_MESSAGE_ID" = %lld", msg_id);
...
void insert_or_update_peer_into_database(tg_peer_info_s* UC)
{
	...
	if (!ret) {
		...
		char where_clause[1024];
		sprintf(where_clause, PEER_INFO_TABLE_CHAT_ID" = %d", UC->peer_id);
		ret = update_table(table_name, col_names, col_types, col_values, where_clause);
	} else {
...
void insert_media_info_to_db(tg_message_s *M, char *file_path, int width, int height, int size, char *latitude, char *longitude, char *first_name, char *last_name, char *phone_number)
{
	...
	if (media_type == tgl_message_media_photo) {
		...
		col_values = eina_list_append(col_values, g_zero_char);
...
int get_media_size_from_db(long long media_id)
{
	...
	char where_clause[1024];
	sprintf(where_clause, MEDIA_INFO_TABLE_MEDIA_ID" = '%lld'", media_id);
...
char *get_media_path_from_db(long long media_id)
{
	...
	if (!vals) {
		...
		// there will be only one item and one row
		Eina_List *first_row = eina_list_nth(vals, 0);
		char *temp_file_path = (char *)eina_list_nth(first_row, 0);
		if (temp_file_path)
			file_path = strdup(temp_file_path);
...
Eina_Bool mark_all_records_for_deletion(char *tablename)
{
	...
	char var_query[1024];
	sprintf(var_query, "UPDATE %s SET "MESSAGE_INFO_TABLE_MARKED_FOR_DELETE" = 1 WHERE "MESSAGE_INFO_TABLE_MARKED_FOR_DELETE" = 0;", tablename);
...
Eina_Bool delete_date_messages_from_table(char *tablename)
{
	...
	char var_query[1024];
	sprintf(var_query, "DELETE FROM %s WHERE "MESSAGE_INFO_TABLE_SERVICE" = 2;", tablename);
...
Eina_Bool mark_group_chat_as_unknown(int chat_id)
{
	...
	char where_clause[1024];
	sprintf(where_clause, PEER_INFO_TABLE_CHAT_ID" = %d", chat_id);
